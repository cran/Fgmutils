...
estatisticasCorrPERCENTUAL <- function(observado, estimado, dfEstatisticas, ...){
  if(!("corr" %in% dfEstatisticas$estatisticasDoModelo$name))
    stop("ERROR: FIRST CALC CORR")
  dfEstatisticas$estatisticasDoModelo = rbind(dfEstatisticas$estatisticasDoModelo, data.frame(name = "corr_PERCENTUAL", value =
                                                                                       calculaPerc(dfEstatisticas$estatisticasDoModelo[dfEstatisticas$estatisticasDoModelo$name=="corr", "value"], observados = observado)))
